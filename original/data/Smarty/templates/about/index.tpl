<div class="about-index">
	<form action="{$HTML}about/complete.php" method="post">
		<div class="about-index-area">
			<div class="about-index-title">
				<h1>以下の内容でお問い合わせを送信します。</h1>
			</div>
			<div class="about-index-content">
				<form action="{$HTML}about/complete.php" method="post">
				<p class="about-subtitle">お名前</p>
				<p class="about-subcontent">afklsfsafs</p>
				<p class="about-subtitle">メールアドレス</p>
				<P class="about-subcontent">fsafsfsda</P>
				<p class="about-subtitle">内容</p>
				<p class="about-subcontent">sfsafsfs</p>
			</div>
		</div>
		<div class="about-button-area">
			<input type="submit" value="送信する" class="about-index-button">
		</div>
	</form>
</div>